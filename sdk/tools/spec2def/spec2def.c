...
PrintName(FILE *fileDest, EXPORT *pexp, PSTRING pstr, int fDeco)
{
    ...
    else if ((nNameLength == 1) && (pcName[0] == '@'))
    {
    ...
    va_list argptr)
{
    ...
    for (len = 0; pcLine[len] != '\x0D' && pcLine[len] != '\x0A'; len++)
        (void)0;
    ...
    *cExports = 0;
    ...
    for (pcLine = pcStart; *pcLine; pcLine = NextLine(pcLine), nLine++)
    {
            ...
            exp.nCallingConvention != CC_STUB)
        {
            ...
            if (*pc++ != '(')
            {
                Fatal(pszSourceFileName, nLine, pcLine, pc, 0, "Expected '('");
            }
...
int main(int argc, char *argv[])
{
    ...
    if (pexports == NULL)
    {
        fprintf(stderr, "Empty spec file\n");
        return 0;
    }
