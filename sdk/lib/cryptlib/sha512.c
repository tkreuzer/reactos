...
#define ULL(a,b)   (((unsigned long long)(a) << 32) | (b))
...
static void processblock(SHA512_CTX *ctx, const unsigned char *buffer)
{
    unsigned long long W[80], t1, t2, a, b, c, d, e, f, g, h;
    int i;

    for (i = 0; i < 16; i++)
    {
        W[i]  = (unsigned long long)buffer[8*i]<<56;
        W[i] |= (unsigned long long)buffer[8*i+1]<<48;
        W[i] |= (unsigned long long)buffer[8*i+2]<<40;
        W[i] |= (unsigned long long)buffer[8*i+3]<<32;
        W[i] |= (unsigned long long)buffer[8*i+4]<<24;
        W[i] |= (unsigned long long)buffer[8*i+5]<<16;
        W[i] |= (unsigned long long)buffer[8*i+6]<<8;
        W[i] |= buffer[8*i+7];
    }
...
{
    unsigned long long r = ctx->len % 128;
...
void sha512_update(SHA512_CTX *ctx, const unsigned char *buffer, unsigned long  len)
{
...
void sha512_finalize(SHA512_CTX *ctx, unsigned char *buffer)
{
    int i;
...
void sha384_finalize(SHA512_CTX *ctx, unsigned char *buffer)
{
    unsigned char buffer512[64];
