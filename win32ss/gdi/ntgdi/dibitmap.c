...
DibGetBitmapInfoSize(
    ...
    _In_ ULONG iUsage)
{
    ULONG cjHeaderSize, cjColorSize, cBitsPixel, cColors, cMaxColors;
    ...
    cjColorSize = (iUsage == DIB_RGB_COLORS) ? sizeof(RGBQUAD) :
                  (iUsage == DIB_PAL_INDICES) ? 0 :
                  sizeof(WORD);
    ...
    if (cjHeaderSize < sizeof(BITMAPINFOHEADER))
    {
        ...
        return sizeof(BITMAPCOREHEADER) + cColors * cjColorSize;
    }
...
/// FIXME: should be able to capture a full packed DIB
_Success_(return!=NULL)
_Ret_bytecount_(*pcjInfo)
PBITMAPINFO
NTAPI
DibProbeAndCaptureBitmapInfo(
    _In_reads_bytes_(*pcjInfo) const BITMAPINFO *pbmiUnsafe,
    _In_ ULONG iUsage,
    _Inout_ PULONG pcjInfo)
{
    BITMAPINFO *pbmi;
