...
DC_vFixIsotropicMapping(PDC pdc)
{
    ...
    fx = _abs64((LONG64)szlWindowExt.cx * (LONG64)szlViewportExt.cy);
    fy = _abs64((LONG64)szlWindowExt.cy * (LONG64)szlViewportExt.cx);
    ...
    INT iMode)
{
    ...
    Points = (LPPOINT)ExAllocatePoolWithTag(PagedPool, Size, GDITAG_TEMP);
    if (!Points)
    {
        DC_UnlockDc(pdc);
