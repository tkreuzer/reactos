                   ...
                   DWORD  Count)
{
    ...
    {
        POINT *npt;
        npt = (PPOINTL)ExAllocatePoolWithTag(PagedPool,
               ...
               int     Count)
{
    ...
    if (!(pbrLine->flAttrs & BR_IS_NULL))
    {
        Points = (PPOINTL)EngAllocMem(0, Count * sizeof(POINT), GDITAG_TEMP);
        if (Points != NULL)
        {
                 ...
                 DWORD   Count)
{
    ...
    {
        POINT *pts = (PPOINTL)ExAllocatePoolWithTag(PagedPool,
                                           sizeof(POINT) * (Count + 1),
                                           TAG_SHAPE);
        if ( pts )
        {
    ...
    IN ULONG cCount)
{
    ...
    _SEH2_TRY
    {
        ...
        space = cCount + 300;
        line_pts = (PPOINTL)ExAllocatePoolWithTag(PagedPool, space * sizeof(POINT), TAG_SHAPE);
        if (line_pts == NULL)
        {
            result = FALSE;
            _SEH2_LEAVE;
