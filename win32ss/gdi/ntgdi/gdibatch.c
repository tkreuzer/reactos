...
EXTERN_C BOOL APIENTRY IntExtTextOutW(IN PDC, IN INT, IN INT, IN UINT, IN OPTIONAL PRECTL, IN LPCWSTR, IN INT, IN OPTIONAL LPINT, IN DWORD);
...
GdiFlushUserBatch(PDC dc, PGDIBATCHHDR pHdr)
{
  ...
  switch(Cmd)
  {
     ...
     case GdiBCTextOut:
     {
        ...
        if ((flags & DIRTY_TEXT) && (crColor != (ULONG)-1))
        {
            dc->pdcattr->crForegroundClr = crColor;
            dc->pdcattr->ulForegroundClr = ulForegroundClr;
        }
