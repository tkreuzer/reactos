    ...
    _Outptr_ PEVENT *ppEvent)
{
    ...
    if (EngEvent)
    {
        ...
        KeInitializeEvent((PRKEVENT)EngEvent->pKEvent,
...
{
    /* Clear the event */
    KeClearEvent((PRKEVENT)Event->pKEvent);
}
...
{
    /* Set the event */
    return KeSetEvent((PRKEVENT)Event->pKEvent,
                      IO_NO_INCREMENT,
                      FALSE);
}
    ...
    _In_ PEVENT Event)
{
    ...
    return KeReadStateEvent((PRKEVENT)Event->pKEvent);
}
    ...
    _Reserved_ PVOID Reserved3)
{
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        KePulseEvent((PRKEVENT)pvEvent, EVENT_INCREMENT, FALSE);
