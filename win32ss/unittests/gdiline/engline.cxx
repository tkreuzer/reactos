...

{
    ...
    // left case.
        ...
        // Reset some variables we'll nuke a little later:
            ...
            euq = UInt32x32To64(x0, dN) + llGamma;
...
\***********************************************************************/
    ...
    // Flip back to device space:
        ...
        if (2 * dN > dM)
        {
            ...
            llBeta  = llGamma - (LONGLONG) (ULONGLONG) dM;
            dN = dM - dN;
    ...
    // go in the very first strip:

        {
            ...
            {
                euq = UInt32x32To64(y0 + 1, dM) + llBeta;

                ASSERTDD(dl >= 0, "Oops!  Miscalculated y0");
            ...
            ASSERTDD(i > 0 && i <= LONG_MAX, "Weird start strip length");
            ASSERTDD(cPels > 0, "Zero cPels");
