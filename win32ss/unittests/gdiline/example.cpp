...
//
// Consider the line from (X0/F, Y0/F) to (X1/F, Y1/F) which runs
// left-to-right and whose slope is in the first octant, and let
// dX = X1 - X0 and dY = Y1 - Y0.  Then dX >= 0, dY >= 0 and dX >= dY.
//
...
//
//             = floor( ((dX / dY) (j F - F/2 - Y0) + X0) / F ) + 1
//
//             = floor( F dX j - F/2 dX - Y0 dX + dY X0) / F dY ) + 1
//
//                      F dX j - [ dX (Y0 + F/2) - dY X0 ]
//             = floor( ---------------------------------- ) + 1
//                                   F dY
//
//                      dX j - [ dX (Y0 + F/2) - dY X0 ] / F
//             = floor( ------------------------------------ ) + 1       (1)
//                                     dY
//
//             = floor( (dX j + alpha) / dY ) + 1
//
// where
//
//       alpha = - [ dX (Y0 + F/2) - dY X0 ] / F
//
...
// alpha:
//
//       beta = floor( - [ dX (Y0 + F/2) - dY X0 ] / F )                 (2)
//
// so
//
//       iL(j) = floor( (dX j + beta) / dY ) + 1                         (3)
//
// for integers j.
//
...
// lit:
//
//                       dY i + [ dX (Y0 + F/2) - dY X0 ] / F
//       j(i) = ceiling( ------------------------------------ ) - 1
//                                       dX
//
// Floors are easier to compute, so we can rewrite this:
//
//                     dY i + [ dX (Y0 + F/2) - dY X0 ] / F + dX - 1/F
//       j(i) = floor( ----------------------------------------------- ) - 1
//                                       dX
//
//                     dY i + [ dX (Y0 + F/2) - dY X0 ] / F + dX - 1/F - dX
//            = floor( ---------------------------------------------------- )
//                                       dX
//
//                     dY i + [ dX (Y0 + F/2) - dY X0 - 1 ] / F
//            = floor( ---------------------------------------- )
//                                       dX
//
...
//
//       j(i) = floor( (dY i + gamma) / dX )                             (4)
//
// where
//
//       gamma = floor( [ dX (Y0 + F/2) - dY X0 - 1 ] / F )              (5)
//
// We now note that
//
...
//     j
//
//       = floor((dX (j + 1) + beta) / dY) - floor((dX j + beta) / dY)   (7)
//
// This may be rewritten as
//
//    c  = floor(i    + r    / dY) - floor(i  + r  / dY)                 (8)
//     j          j+1    j+1                j    j
//
// where i , i    are integers and r  < dY, r    < dY.
//        j   j+1                   j        j+1
//
// Rewriting (7) again:
//
//    c  = floor(i  + r  / dY + dX / dY) - floor(i  + r  / dY)
//     j          j    j                          j    j
//
//
//       = floor((r  + dX) / dY) - floor(r  / dY)
//                 j                      j
//
// This may be rewritten as
//
//    c  = dI + floor((r  + dR) / dY) - floor(r  / dY)
//     j                j                      j
//
// where dI + dR / dY = dX / dY, dI is an integer and dR < dY.
//
// r  is the remainder (or "error") term in the DDA loop: r  / dY
//  j                                                      j
...
//
//    c  = dI + floor((r  + dR) / dY) and r    = (r  + dR) % dY
//     j                j                  j+1     j
//
// and we know r  < dY, r    < dY, and dR < dY.
//              j        j+1
//
...
//
// We move the origin to (floor(X0 / F), floor(Y0 / F)), so when we
// calculate gamma from (5), we know that 0 <= X0, Y0 < F.  And we
// are in the first octant, so dX >= dY.  Then we know that gamma can
// be in the range [(-1/2)dX, (3/2)dX].  The DDI guarantees us that
// valid lines will have dX and dY values at most 31 bits (unsigned)
...
//                                     j
// range 0 <= r  < 2 dY; thus the result must be a 32 bit unsigned value.
//             j
//
// Testing Lines
// -------------
...
{
    ULONG X0, dX, Y0, dY, dY_Original;
    FLONG fl;
    LONG x, y;

    LONGLONG  llBeta, llGamma;
    LONGLONG  dl, ll;

    ULONG ulDelta;

    ULONG x0, y0, x1, xStart;
    ...
    do
    {
        ...
        //
        X0 = (LONG) pptfxFirst->x;
        dX = (LONG) pptfxBuf->x;

        Y0 = (LONG) pptfxFirst->y;
        dY = (LONG) pptfxBuf->y;

        fl = flStart;

        DBG_GDI((7, "Lines: Slow Lines %x %x %x %x", X0, Y0, dX, dY));
        ...
        dX -= X0;
        if ((LONG) dX < 0)      // We can skip any lines with delta > 2^31 - 1
        {
            goto Next_Line;
        }
        ...
        dY -= Y0;
        if ((LONG) dY < 0)      // We can skip any lines with delta > 2^31 - 1
        {
            goto Next_Line;
        }

        // We now have a line running left-to-right, top-to-bottom from (X0, Y0)
        // to (X0 + dX, Y0 + dY):
        ...
        //
        // Convert X0 and Y0 from 28.4 format to normal interger
        //
        x = LFLOOR((LONG)X0);
        y = LFLOOR((LONG)Y0);
        ...
        // Calculate the remainder term [ dX * (Y0 + F/2) - X0 * dY ]:

        llGamma = Int32x32To64(dX, Y0 + FBITS/2) - Int32x32To64(X0, dY);
        ...
        // end points, and call them (X0, Y0) and (X1, Y1) respectively, where
        // 0 <= X0, Y0, X1, Y1 < 16.  We plot (X0, Y0) on the following grid
        // to determine x0:
        //
        //   +-----------------------> +x
        //   |
        ...
        // exclusion.  If (X0, Y0) lands on an 'x', x0 = 2.  If (X0, Y0) lands
        // on a '.', x0 = 1.  If (X0, Y0) lands on a '?', x0 rounds up or down,
        // depending on what flips have been done to normalize the line.
        //
        // For the end point, if (X1, Y1) lands on an 'x', x1 =
        // floor((X0 + dX) / 16) + 1.  If (X1, Y1) lands on a '.', x1 =
        // floor((X0 + dX)).  If (X1, Y1) lands on a '?', x1 rounds up or down,
        // depending on what flips have been done to normalize the line.
        //
        // Lines of exactly slope one require a special case for both the start
        // and end.  For example, if the line ends such that (X1, Y1) is (9, 1),
        ...
        //
        // This grid accounts for the appropriate rounding of GIQ and last-pel
        // exclusion.  If (X0, Y0) lands on an 'x', x0 = 0.  If (X0, Y0) lands
        // on a '.', x0 = 1.  If (X0, Y0) lands on a '?', x0 rounds up or down,
        // depending on what flips have been done to normalize the line.
        //
        // For the end point, if (X1, Y1) lands on an 'x', x1 =
...

        {
            ...
            ULONG Y1 = FXFRAC(Y0 + dY);
            ULONG X1 = FXFRAC(X0 + dX);
            ...
            if (fl & FL_FLIP_H)
            {
                ...
                if (Y1 == 0)
                {
                    if (LROUND(X1, fl & FL_H_ROUND_DOWN))
                    {
                        x1++;
                    }
                }
                else if (ABS((LONG) (Y1 - FBITS/2)) + X1 > FBITS)
                {
                    x1++;
                }
                ...
                if (Y0 == 0)
                {
                    if (LROUND(X0, fl & FL_H_ROUND_DOWN))
                    {
                        x0      = 2;
                        ulDelta = dY;
                    }
                }
                else if (ABS((LONG) (Y0 - FBITS/2)) + X0 > FBITS)
                {
                    x0      = 2;
                    ulDelta = dY;
                }
                ...
                if (ll >= (LONGLONG) (2 * dX - dY))
                    y0 = 2;
                else if (ll >= (LONGLONG) (dX - dY))
                    y0 = 1;
            }
            else
            {
                ...
                if (X1 > 0)
                {
                    ...
                    else if (ABS((LONG) (Y1 - FBITS/2)) <= (LONG) X1)
                    {
                        x1++;
                    }
                }
                ...
                if (X0 > 0)
                {
                    ...
                    else if (ABS((LONG) (Y0 - FBITS/2)) <= (LONG) X0)
                    {
                        x0 = 1;
                    }
                }
                ...
                y0 = 0;
                if (llGamma >= (LONGLONG) (dX - (dY & (-(LONG) x0))))
                {
                    y0 = 1;
                }
            }
        ...
        if (fl & FL_COMPLEX_CLIP)
        {
            ...
            dl = Int32x32To64(x0, dY) + llGamma;

            y0 = UInt64Div32To32(dl, dX);
        ...
        if (fl & FL_SIMPLE_CLIP)
        {
            ...
            {
                y1 = yBottom;
                dl = Int32x32To64(y1, dX) + llBeta;

                x1 = UInt64Div32To32(dl, dY);
            }
            ...
            {
                y0 = yTop;
                dl = Int32x32To64(y0, dX) + llBeta;

                x0 = UInt64Div32To32(dl, dY) + 1;
            ...
            !(fl & FL_DONT_DO_HALF_FLIP))
        {
            ...
            llBeta  = llGamma - (LONGLONG) ((LONG) dX);
            dY = dX - dY;
...

        {
            ...
            {
                dl = Int32x32To64(y0 + 1, dX) + llBeta;

                ASSERTDD(dl >= 0, "Oops!");

                i = UInt64Div32To32(dl, dY);
                r = UInt64Mod32To32(dl, dY);
                i = i - x0 + 1;
