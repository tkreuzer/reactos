...
START_TEST(RtlCaptureContext)
{
    ...
    ok_eq_hex(CapturedContext.FltSave.ErrorOpcode, 0x00000000);
    ...
    ok_eq_hex(CapturedContext.FltSave.MxCsr_Mask, 0xcccc);
    ok_eq_hex(CapturedContext.FltSave.ErrorOpcode, 0xcccc);
    ...
    RtlFillMemory(&CapturedContext, sizeof(CapturedContext), 0xCC);
    RtlCaptureContextWrapper(&InOutContext, &CapturedContext);
    /* Some EFlags fields are set (parity is flaky)  ok_eq_hex64(CapturedContext.EFlags & ~0x4, 0x602);
