    ...
    _In_ PBOOLEAN CheckForPmp)
{
    ...
    SrbStatus = AtaAhciSendResetFis(ChanData, AHCI_PMP_CONTROL_PORT, FALSE, 2000);
    if (SrbStatus != SRB_STATUS_SUCCESS)
    {
        INFO("CH %lu: Soft reset failed %02x\n", ChanData->Channel, SrbStatus);
    ...
    _In_ ULONG PortNumber)
{
    ...
    /* Clear errors */
    SrbStatus = AtaAhciPmpWrite(ChanData, PortNumber, ATA_SERROR, 0xFFFFFFFF);
    if (SrbStatus != SRB_STATUS_SUCCESS)
        return FALSE;

    SataControl = AHCI_PXCTL_DET_RESET;
    SrbStatus = AtaAhciPmpWrite(ChanData, PortNumber, ATA_SCONTROL, SataControl);
    if (SrbStatus != SRB_STATUS_SUCCESS)
        return FALSE;
    ...
    _In_ ULONG PortNumber)
{
    ...
    {
        SrbStatus = AtaAhciPmpRead(ChanData, PortNumber, ATA_SSTATUS, &SataStatus);
    ...
    _In_ PCHANNEL_DATA_AHCI ChanData)
{
    AtaAhciStopCommandListProcessAndWait(ChanData);
    AtaAhciPerformCommandListOverride(ChanData);
    AtaAhciStopFisReceiveProcessAndWait(ChanData);
}
    ...
    _In_ BOOLEAN ProcessErrorStatus)
{
    ...
    else
    {
        ...
        if (Request->Flags & REQUEST_FLAG_NCQ)
        {
             ...
             */
            RtlCopyMemory(&Request->Output, &Request->TaskFile, sizeof(Request->TaskFile));
