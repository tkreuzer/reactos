...
TIFFOpen(const char* name, const char* mode)
{
	...
	tif = TIFFFdOpen((intptr_t)fd, name, mode);   /* FIXME: WIN64 cast from pointer to int warning */
	if(!tif)
		CloseHandle(fd);
	return tif;
}
...
TIFFOpenW(const wchar_t* name, const char* mode)
{
	...
	tif = TIFFFdOpen((intptr_t)fd,    /* FIXME: WIN64 cast from pointer to int warning */
