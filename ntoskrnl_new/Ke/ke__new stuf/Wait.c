    ...
    PDISPATCHER_HEADER DispatcherHeader)
{
    ...
    /* Nothing more to do for non-synchronisation objects */
    ...
    __in PLARGE_INTEGER Timeout)
{
    ...
    while (TRUE)
    {
        ...
        /* If we got here, we were woken up by a signaled synchronisation
    ...
    KPRIORITY PriorityIncrement)
{
    ...
    while (TRUE)
    {
        ...
        /* Set the wait blocks state to inactive. This way the thread
           can avoid locking the dispatcher object again to unqueue itself */
        WaitBlock->BlockState = WaitBlockInactive;
