                                ...
                                IN PLDR_DATA_TABLE_ENTRY ScanDTE)
{
	...
	for (;(ImportTable->Name != 0) && (ImportTable->FirstThunk != 0);ImportTable++)
	{
        ...
        if (!Success)
		{
           ERR("WinLdrpScanImportAddressTable() failed for scanDTE='%wZ', ImportName='%s'\n",
                ImportName, &ScanDTE->BaseDllName);
            return Success;
		}
	}
                      ...
                      IN PCSTR DirectoryPath)
{
	...
	else
	{
		...
		ImportByName = (PIMAGE_IMPORT_BY_NAME)VaToPa((PVOID)ThunkData->u1.AddressOfData);
		...
		if ((Hint < ExportDirectory->NumberOfNames) &&
			(strcmp(&ImportByName->Name[0],
			        VaToPa(RVA(DllBase, NameTable[Hint])) ) == 0) )
		{
		...
		else
		{
			...
			while (High >= Low)
			{
				...
				/* Compare the names */
				Result = strcmp(&ImportByName->Name[0],
					            VaToPa(RVA(DllBase, NameTable[Middle])));
